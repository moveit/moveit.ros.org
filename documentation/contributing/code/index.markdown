...
In some cases, the warnings generated by clang-tidy may be false-positive, e.g. `performance-unnecessary-copy-initialization` may falsely convert `non-const` variable to `const` variable, or cannot be silenced for some reason.

It is recommanded to suppress the undesired clang-tidy checks by using `NOLINT` and `NOLINTNEXTLINE` comments followed by the list of check names in a parentheses:
```c++
...
for (const std::pair<const std::string, std::int> & item : m)
