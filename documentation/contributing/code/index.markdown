...
In some cases, the warnings generated by clang-tidy may be false-positive, e.g. `performance-unnecessary-copy-initialization` may falsely convert a `non-const` variable to `const`, or cannot be silenced for some reason.
